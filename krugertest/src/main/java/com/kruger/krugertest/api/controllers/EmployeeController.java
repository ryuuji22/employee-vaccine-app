...
import com.kruger.krugertest.application.command.edit.EditEmployeeCommand;
import com.kruger.krugertest.application.queries.ReadEmployeeByIdentificationQuery;
...
public class EmployeeController {
    ...
    public ResponseEntity<Employee> readEmployee() {
		ReadEmployeeByIdentificationQuery query = new ReadEmployeeByIdentificationQuery();
        logger.info("----- Sending Query: {} ", query.getClass().getName());
        return new ResponseEntity<>(this.mediator.dispatch(query), HttpStatus.OK);
    }
	
	@PatchMapping
	public ResponseEntity<Void> editEmployee(@Valid @RequestBody EditEmployeeCommand command) {
